...
Reads MakeMeAHanzi-style all.json,
produces a SQLite database with binary-encoded stroke data.
Characters table only stores unicode + character symbol — pinyin/definition
are loaded at runtime from CSV character sets.
"""
...
def build_database(data_dir: str, output_path: str, db_version: int = 3):
    data_dir = Path(data_dir)
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    # Load all character data from JSON
    characters_data = {}
    ...
    for char, char_data in characters_data.items():
        ...
        cursor.execute("""
            INSERT OR REPLACE INTO characters (unicode, char) VALUES (?, ?)
        """, (unicode, char))
    ...
    # Optimize for read-only usage (must be outside transaction)
...
if __name__ == "__main__":
    ...
    build_database(args.data_dir, args.output, db_version=3)
